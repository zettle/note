...
## 1.2 守护进程的方式启动
`docker run -it --name 别名 镜像ID`: 用守护进程的方式启动，和`-it`的区别是不会进入容器里面
...
## 4、进入容器
`docker attach [容器id或容器名]`: 进入正在运行的容器内部

`docker exec -it [容器id或容器名] [命令]`: 进入容器执行命令，然后退出容器回到宿主机。这种一般用于`redis/nginx`等
