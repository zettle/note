...
## 1、vue2项目升级vue
```shell
npm i -D vue-loader-v16 @vue/compiler-sfc
...
* 以前的`inheritAttrs:false`不会影响class属性，就算你加了`inheritAttrs:false`，className还是会加给子组件最外层，而vue3中，classsName跟着`$attrs`走了
* 以前的v-for优先级比v-if高，而vue3中，v-if的比较高
...
### 3.1 访问app.config.globalProperties属性
以前的`Vue.prototype.xxx`需要改为`app.config.globalProperties.xxx`
```js
